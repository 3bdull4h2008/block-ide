...
| 5.6 | Crash recovery journal | ✅ (pulled forward from P1.6) write-temp-rename journal, boot-time restore prompt — RUN 41 (D13): autosave removed; journal = close-time checkpoint only + rotating backups with boot salvage | kill -9 test manual |
...
| 1.11 | Full research application: sticky category rail (click-jump + scroll-active dots); variable HARVESTING from the open file (declaration nodes; functions/params/fields excluded); right-click rename+delete menu; Make a List -> C arrays (element reporter `name[0]` fits any round socket — slot validator accepts indexed idents in number sockets) | ✅ (RUN 30) | vitest harvest/lists/socket suite; G-UI-E2E rail+make-list+harvest |
...
| D13 | **SAVE MODEL REWORKED 2026-08-25 (user): commercial-grade explicit saving — AUTOSAVE REMOVED.** Keystroke/timer journaling is gone; the ONLY journal write is a close-time checkpoint when the window exits with unsaved work (boot recovery + backup salvage stay as the crash net). Navigation that replaces the single buffer (tab/file/recent/folder/new/open) prompts "discard unsaved changes?"; Ctrl+S saves, **Ctrl+Shift+S = Save As** (native dialog); New File and Open File work WITHOUT any folder open (standalone absolute-path docs via user-confirmed native dialogs); title shows `name • - Cade` when dirty. |
