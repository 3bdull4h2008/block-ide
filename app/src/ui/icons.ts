...
/** Swap emoji labels on existing toolbar / splash controls at boot. */
export function applyChromeIcons(): void {
  const map: Array<[string, string]> = [
    ['open-folder', iconLabel(icons.folder, 'Open')],
    ['open-file', iconLabel(icons.file, 'File')],
    ['new-file', iconLabel(icons.filePlus, 'New')],
    ['save', iconLabel(icons.save, 'Save')],
    ['run', iconLabel(icons.play, 'Run')],
    ['academy-start', iconLabel(icons.academy, 'Start Academy')],
    ['stage-stop', icons.stop],
    ['trace-play', icons.play],
    ['trace-step', icons.step],
    ['trace-reset', icons.rewind],
  ]
  ...
  // Theme toggle: sun/moon glyph driven by current data-theme
  ...
  // Splash nav: keep emoji-free
  const splashMap: Record<string, string> = {
    sandbox: `${icons.newDoc}<span>New</span>`,
    academy: `${icons.academy}<span>Academy</span>`,
    recent: `${icons.recent}<span>Recent</span>`,
    settings: `${icons.settings}<span>Settings</span>`,
  }
  document.querySelectorAll<HTMLElement>('.sidebar-row[data-panel]').forEach((row) => {
    const key = row.dataset.panel
    if (key && splashMap[key]) row.innerHTML = splashMap[key]
  })
}
